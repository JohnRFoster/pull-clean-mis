...
pre.process.data <- function(in.dat) {
  ...
  ifelse(
    ...
    RESET <- rep(0, nrow(data.wide))
  )
  ifelse(
    "UNSET" %in% colnames(data.wide),
    print("UNSET in dataframe"),
    UNSET <- rep(0, nrow(data.wide))
  )
  ifelse(
    "SET" %in% colnames(data.wide),
    print("SET in dataframe"),
    SET <- rep(0, nrow(data.wide))
  )
  ifelse(
    "REMOVED" %in% colnames(data.wide),
    print("REMOVED in dataframe"),
    REMOVED <- rep(0, nrow(data.wide))
  )
  ifelse(
    "APPLIED.USED" %in% colnames(data.wide),
  ...
  ifelse(
    ...
    data.wide <- cbind(data.wide, RESET)
  )
  ifelse(
    "UNSET" %in% colnames(data.wide),
    print("Added UNSET in dataframe"),
    data.wide <- cbind(data.wide, UNSET)
  )
  ifelse(
    "SET" %in% colnames(data.wide),
    print("Added SET to dataframe"),
    data.wide <- cbind(data.wide, SET)
  )
  ifelse(
    "REMOVED" %in% colnames(data.wide),
    print("Added REMOVED to dataframe"),
    data.wide <- cbind(data.wide, REMOVED)
  )

  ifelse(
    "APPLIED.USED" %in% colnames(data.wide),
    print("Did not add Applied.Used in dataframe"),
  ...
  out.dat <- out.dat[
    order(out.dat$AGRP_PRP_ID, out.dat$WT_WORK_DATE),
    ,
    drop = FALSE
  ]
...
calc.days.elapsed <- function(in.dat) {
  ...
  for (i in 2:nrow(in.dat)) {
    in.dat[i, "day.diff"] <- in.dat[i, "WT_WORK_DATE"] -
      in.dat[i - 1, "WT_WORK_DATE"]
...
add.trap.count <- function(in.dat, event.time.threshold = 25) {
  ...
  for (i in 1:nrow(in.dat)) {
    ...
    if (trap.count[i] == 0 && i > 1) {
      in.dat[i, "trap.count"] <- trap.count[i - 1]
      in.dat[i, "trap.count.event"] <- trap.count.event[i]
    }
    ...
    if (i > 1 && last.event.id != current.event.id) {
      in.dat[i, "days.active"] <- 0
    }

    #--Accounting of time since event
    if (time.since.event > event.time.threshold) {
      in.dat[i, "time.since.event"] <- time.since.event
