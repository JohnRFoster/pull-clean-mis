...
unk.events <- cbind.data.frame(
  unk.events,
  firearms.used = rep("firearms", nrow(unk.events))
)
...
event.vec <- firearms.associated[
  firearms.associated$CMP_NAME %in% cmp.name,
  "event.id"
]

firearms.associated <- firearms.associated[
  firearms.associated$event.id %!in% event.vec,
]
...
#Reset CMP_NAME
firearms.associated[
  firearms.associated$CMP_NAME != "FIREARMS",
  "CMP_NAME"
] <- "OTHER"
plyr::count(firearms.associated$CMP_NAME)
...
tmp.dat[tmp.dat$UOM_NAME == "MINUTES", "WTM_QTY"] <- tmp.dat[
  tmp.dat$UOM_NAME == "MINUTES",
  "WTM_QTY"
] /
  60
tmp.dat[tmp.dat$UOM_NAME == "MINUTES", "UOM_NAME"] <- "HOURS"

count(tmp.dat$CMP_NAME)
count(tmp.dat$CMP_TYPE)
count(tmp.dat$USET_NAME)
...
for (i in 1:length(event.vec)) {
  ...
  tmp.dat[
    tmp.dat$event.id == event.vec[i] & tmp.dat$CMP_NAME == "OTHER",
    "WTM_QTY"
  ] <- other.vec
} #END Loop
...
tmp.time[tmp.time$FIREARMS == tmp.time$OTHER, "HOURS"] <- tmp.time[
  tmp.time$FIREARMS == tmp.time$OTHER,
  "FIREARMS"
]
...
harvest.chronology[
  harvest.chronology$FIREARMS == harvest.chronology$Take &
    harvest.chronology$FIREARMS > 1,
  "FIREARMS"
] <- 1


#----Calculate Hunt Hours and Hunt Days
harvest.chronology$Hunt.Hours <- harvest.chronology$HOURS *
  harvest.chronology$FIREARMS
...
final.agg.out.dat <- final.agg.out.dat[
  order(final.agg.out.dat$AGRP_PRP_ID, final.agg.out.dat$WT_WORK_DATE),
]
...
#--Add unk id
unk.prp.event.id <- paste0(
  final.agg.out.dat$AGRP_PRP_ID,
  ".",
  seq(1, nrow(final.agg.out.dat), 1)
)

#--Reorder
final.agg.out.dat <- data.frame(append(
  final.agg.out.dat,
  list(unk.prp.event.id = unk.prp.event.id),
  after = match("ALWS_AGRPROP_ID", names(final.agg.out.dat))
))
...
missing.agrp.id <- unique(harvest.chronology$AGRP_PRP_ID[
  harvest.chronology$AGRP_PRP_ID %!in% lut.property.acres$AGRP_PRP_ID
])

write.csv(
  missing.agrp.id,
  file.path(write.path, "missing.agrp.id.csv"),
  row.names = FALSE
)


dat.Eff[
  dat.Eff$AGRP_PRP_ID == "285573" & dat.Eff$WT_WORK_DATE == "2008-06-16",
]
