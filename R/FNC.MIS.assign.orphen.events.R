...
assign.orphen.events <- function(master.dat, max.time) {
  ...
  for (j in 1:length(property.vec)) {
    ...
    if (length(event.vec) != 0) {
      index.lut <- cbind.data.frame(
        row.number = seq(1, nrow(in.dat), 1),
        rowname = rownames(in.dat)
      )

      for (k in 1:length(event.vec)) {
        if (length(rownames(in.dat[in.dat$event.id == event.vec[k], ])) == 1) {
          row.location <- index.lut[
            index.lut$rowname ==
              rownames(in.dat[in.dat$event.id == event.vec[k], ]),
            "row.number"
          ]
          ...
          if (row.location != 1) {
            length.t.minus1 <- abs(
              (in.dat[row.location, "WT_WORK_DATE"] -
                in.dat[row.location - 1, "WT_WORK_DATE"])
            )
          }
          ...
          if (row.location == 1) {
            if (length.t.plus1 < max.time) {
              in.dat[row.location, "event.id"] <- in.dat[
                row.location + 1,
                "event.id"
              ]
              in.dat[
                row.location,
                "time.since.event.type"
              ] <- "MERGED WITH NEXT"
            }
          }
            ...
            if (length.t.minus1 < max.time && length.t.plus1 < max.time) {
              if (length.t.minus1 < length.t.plus1) {
                if (length.t.minus1 < max.time) {
                  ...
                  in.dat[row.location, "time.since.event.type"] ==
                    "MERGED WITH PREVIOUS"
                }
              }
    ...
    if (j == 1) {
      out.dat <- rbind.data.frame(not.process.dat, in.dat)
    }
    if (j > 1) {
      out.dat <- rbind.data.frame(out.dat, in.dat)
    }
