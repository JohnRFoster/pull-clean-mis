...
aerial.chronology <- function(in.dat, time.thershold = 25) {
  #----Load Libraries----
  ...
  if (nrow(tmp.dat) > 1) {
    ...
    for (i in 2:nrow(tmp.dat)) {
      tmp.dat[i, "day.diff"] <- tmp.dat[i, "WT_WORK_DATE"] -
        tmp.dat[i - 1, "WT_WORK_DATE"]

      if (tmp.dat[i, "day.diff"] > time.thershold) {
        tmp.dat[i, "time.since.event"] <- tmp.dat[i, "day.diff"]
      }

      #--Assign event ID
      if (i == 2) {
        tmp.dat[i - 1, "event.id"] <- event.id
      }
      if (tmp.dat[i, "time.since.event"] < 1) {
        tmp.dat[i, "event.id"] <- event.id
      }
      if (tmp.dat[i, "time.since.event"] > 0) {
        event.id = event.id + 1
        tmp.dat[i, "event.id"] <- event.id
      }
