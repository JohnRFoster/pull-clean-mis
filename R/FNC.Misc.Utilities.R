calc.event.length <- function(in.dat) {
  ...
  count.within.event.actions <- plyr::count(in.dat[, c(
    "AGRP_PRP_ID",
    "event.id",
    "unk.prp.event.id",
    "ALWS_AGRPROP_ID",
  ...
  colnames(count.within.event.actions)[ncol(
    count.within.event.actions
  )] <- "num.actions"

  max.date <- aggregate(
    WT_WORK_DATE ~ AGRP_PRP_ID +
      event.id +
      unk.prp.event.id +
      ALWS_AGRPROP_ID +
      CMP_NAME,
  ...
  date.lut$information.quaility <- date.lut$num.actions /
...
calc.days.between.records <- function(in.dat) {
  ...
  for (k in 1:length(property.vec)) {
    ...
    if (nrow(tmp) > 1) {
      for (i in 2:nrow(tmp)) {
        day.diff.vec[i] <- tmp$WT_WORK_DATE[i] - tmp$WT_WORK_DATE[i - 1]
      }
    }
...
calc.start.stop.by.record <- function(in.dat, adjustment = 0) {
  ...
  for (k in 1:length(property.event.vec)) {
    ...
    for (i in 1:nrow(tmp)) {
      ...
      if (i > 1) {
        within.event.str.date[i] <- as.character(
          tmp$WT_WORK_DATE[i] - (tmp$day.diff[i] - adjustment)
        )
        within.event.end.date[i] <- as.character(tmp$WT_WORK_DATE[i])
      }
    } #END LOOP
...
make.property.lut <- function(dat.Agr) {
  ...
  lut.property.acres <- generate.mean.property.size(
    ...
    area.col = "TOTAL.LAND",
    area.thershold = 2,
    adj.file = "data/county_adjacency2010.csv"
  )
...
generate.trap.chronology <- function(
  trap.dat,
  dat.PropKill,
  trap.vec,
  time.thershold
  ...
  for (k in 1:length(trap.vec)) {
    ...
    if (k > 1) {
      out.harvest.chronology <- rbind.data.frame(
        out.harvest.chronology,
        harvest.chronology
      )
    }
  } ##END Loop
  ...
  for (i in 1:nrow(date.lut)) {
    if (date.lut[i, "end.date"] != date.lut[i, "start.date"]) {
      trap.harvest.chronology[
        (trap.harvest.chronology$AGRP_PRP_ID == date.lut[i, "AGRP_PRP_ID"] &
          trap.harvest.chronology$event.id == date.lut[i, "event.id"] &
          trap.harvest.chronology$WT_WORK_DATE == date.lut[i, "start.date"]),
        "event.type"
      ] <- "Event Start"
...
break.up.long.events <- function(
  trap.harvest.chronology,
  long.event.thershold = 5
) {
  date.lut <- calc.event.length(trap.harvest.chronology)
  long.events <- date.lut[
    date.lut$event.length > long.event.thershold,
    "unk.prp.event.id"
  ]

  for (i in 1:length(long.events)) {
    tmp <- trap.harvest.chronology[
  ...
  for (i in 1:length(tmp.vec)) {
    new.event.id <- as.numeric(factor(trap.harvest.chronology[
      trap.harvest.chronology$AGRP_PRP_ID == tmp.vec[i],
      "unk.prp.event.id"
    ]))
    trap.harvest.chronology[
      trap.harvest.chronology$AGRP_PRP_ID == tmp.vec[i],
...
generate.mean.property.size <- function(
  ...
  adj.file[nchar(adj.file$FIPS.neighbor) == 4, "FIPS.neighbor"] <- paste0(
    "0",
    adj.file[nchar(adj.file$FIPS.neighbor) == 4, "FIPS.neighbor"]
  )

  adj.file$FIPS.ST <- substring(adj.file$FIPS, first = 1, last = 2)
  adj.file$FIPS.neighbor.ST <- substring(
    adj.file$FIPS.neighbor,
    first = 1,
    last = 2
  )
  ...
  for (i in 1:length(fips.vec)) {
    vec <- in.dat |>
      as_tibble() |>
      filter(FIPS == fips.vec[i], TOTAL.LAND > area.thershold) |>
      pull(TOTAL.LAND)
...
adjust.firearm.data <- function(in.dat, thershold = 5) {
  ...
  in.dat[
    in.dat$AGRP_PRP_ID %in% property.vec & in.dat$WTCM_QTY > 2,
    "WTCM_QTY"
  ] <- 1
