...
merge.trap.events <- function(in.dat, event.time.threshold, max.time) {
  ...
  for (j in 1:length(prop.vec)) {
    ...
    for (i in 2:nrow(prop.dat)) {
      if (
        prop.dat[i, "time.since.event"] > event.time.threshold &&
          prop.dat[i, "time.since.event"] < max.time
      ) {
        if (prop.dat[i, "Take"] > 0) {
          if (location.i <= nrow(prop.dat)) {
            ...
            if (location.i == nrow(prop.dat)) {
              prop.dat[location.i, "event.id"] <- prop.dat[
                location.i - 1,
                "event.id"
              ]
              prop.dat[location.i, "time.since.event.type"] ==
    ...
    if (j > 1) {
      out.dat <- rbind.data.frame(out.dat, prop.dat)
    }
  } #END All Data Loop
