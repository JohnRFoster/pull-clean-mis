...
trap.harvest.chronology$days.active <- trap.harvest.chronology$within.event.end.date -
...
plot(
  log(final.agg.out.dat$TOTAL.LAND),
  log(final.agg.out.dat$trap.nights),
  xlab = "log Property Size",
...
write.csv(
  ...
  file.path(
    write.path,
    "feral.swine.effort.take.traps.ALL.csv"
  )
)
