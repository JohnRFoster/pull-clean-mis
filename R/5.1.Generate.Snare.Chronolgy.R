...
for (i in 1:nrow(max.vals)) {
  tmp <- dat.Eff[
    dat.Eff$CMP_NAME == max.vals[i, "CMP_NAME"] &
      dat.Eff$WTCM_QTY < max.vals[i, "WTCM_QTY"],
  ]
...
mean(date.lut$event.length)
median(date.lut$event.length)
mean(date.lut$event.length) + sd(date.lut$event.length)
...
poor.dat <- date.lut[
  date.lut$information.quaility < 0.15,
...
if (nrow(poor.dat) > 0) {
  ...
  trap.harvest.chronology <- trap.harvest.chronology[
    is.na(trap.harvest.chronology$Drop.Flag),
  ]
  trap.harvest.chronology <- trap.harvest.chronology[,
    -ncol(trap.harvest.chronology)
  ]
}
#----END----
...
median(as.numeric(date.lut$event.length))
mean(as.numeric(date.lut$event.length))
...
write.csv(
  ...
  file.path(
    write.path,
    "feral.swine.effort.take.snare.ALL.csv"
  )
)
write.csv(
  trap.harvest.chronology,
