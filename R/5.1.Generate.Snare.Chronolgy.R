...
for (i in seq_len(nrow(max_vals))) {
  tmp <- dat_agr_csv2[
    dat_agr_csv2$CMP_NAME == max_vals[i, "CMP_NAME"] &
      dat_agr_csv2$WTCM_QTY < max_vals[i, "WTCM_QTY"],
  ]
...
library(ggplot2)
p <- ggplot(data = trap_dat, aes(x = WTCM_QTY)) + geom_histogram(binwidth = 1)
p + facet_wrap(~CMP_NAME, scales = "free_y")

#Convert all trap types to the same type
trap_dat <- trap_dat |>
  filter(!is.na(WORK_TASK_UOM)) |>
