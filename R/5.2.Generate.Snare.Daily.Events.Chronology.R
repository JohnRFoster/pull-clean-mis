...
# Read data
dat.Agr <- read_csv(file.path(write.path, "processed_fs_national_property.csv"))
dat.Kill <- read_csv(file.path(
	write.path,
	"processed_fs_national_take_by_method.csv"
))
dat.Eff <- read_csv(file.path(write.path, "processed_fs_national_effort.csv"))
...
trap.harvest.chronology <- read_csv(file.path(
	write.path,
	"feral.swine.effort.take.snare.chronology.ALL.csv"
))
...
trap.harvest.chronology <- trap.harvest.chronology[
	trap.harvest.chronology$within.id != 1,
]

#Days active
trap.harvest.chronology$days.active <- as.numeric(
	trap.harvest.chronology$within.event.end.date -
		trap.harvest.chronology$within.event.str.date
)

#Assume first day active is 1 (for daily summary)
trap.harvest.chronology[
	trap.harvest.chronology$days.active == 0,
	"days.active"
] <- 1

#Calc trap nights
trap.harvest.chronology$trap.nights <- as.numeric(
	trap.harvest.chronology$days.active
) *
	trap.harvest.chronology$trap.count
...
#Reorder things
agg.out.dat <- trap.harvest.chronology[, c(
	"AGRP_PRP_ID",
	"unk.prp.event.id",
	"ALWS_AGRPROP_ID",
	"CMP_NAME",
