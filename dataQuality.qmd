...
make_percentage <- function(x) round(x * 100, 1)

loss_1 <- make_percentage(n_no_state / n_with_pigs)
...
loss_2 <- make_percentage(n_1acre / n_with_pigs)
...
get_numbers <- function(df_raw, df_processed) {
  ...
  out$percent_rows_lost <- make_percentage(out$n_rows_lost / nrow(df_raw))
  ...
  out$percent_props_lost <- make_percentage(out$n_props_lost / out$n_props_raw)
  return(out)
}
```
...
percent_props_with_traps <- make_percentage(traps$n_props_raw / n_with_pigs)
...
aerial.raw <- dat.Eff[dat.Eff$CMP_NAME %in% aerial.vec, ]
dat.aerial <- read.csv(file.path(write.path, "feral.swine.effort.take.aerial.ALL.daily.csv"))

aerial <- get_numbers(aerial.raw, dat.aerial)
...
raw_props <- sum(c(
  traps$n_props_raw,
  firearms$n_props_raw,
  aerial$n_props_raw,
  snare$n_props_raw
))

processed_events <- sum(c(
  traps$n_rows_processed,
  firearms$n_rows_processed,
  aerial$n_rows_processed,
  snare$n_rows_processed
))

processed_props <- sum(c(
  traps$n_props_processed,
  firearms$n_props_processed,
  aerial$n_props_processed,
  snare$n_props_processed
))

percent_events_lost <- make_percentage(1 - processed_events / raw_events)
percent_props_lost <- make_percentage(1 - processed_props / raw_props)

n_ts <- (traps$n_rows_raw + snare$n_rows_raw) / raw_events
ts_percent_raw <- make_percentage(n_ts)
...
process_left <- make_percentage(n_processed / n_with_pigs)

fb_only <- with_fb |>
  filter(farm_bill == 1)

n_fb <- length(unique(fb_only$propertyID))
fb_left <- make_percentage(n_fb / n_farm1)
...
per_left_final <- make_percentage(n_final_props / n_with_pigs)
per_left_final_fb <- make_percentage(n_final_props_fb / n_farm1)

```

## Executive summary

We evaluated MIS data regarding the amount of usable information available to support density estimation using removal (i.e. catch-effort) based approaches. Eleven years of data spanning `r min_date` to `r max_date` was evaluated. This raw (unprocessed) data contains `r n_with_pigs` properties, which includes `r n_farm1` Farm Bill properties. Two evaluations were conducted, 1) evaluation of the amount of errant or missing data, and 2) the amount of data that meets the minimum criteria for density estimation using a removal model. Below is a summary for each method. After processing each method, there is a final processing step to create a data set that is suitable for density estimation.

### Incomplete and errant data

Across all removal methods `r percent_events_lost`% of removal events had data errors or incomplete information. This results in `r percent_props_lost`% of properties being removed from the raw MIS data (Table 1).

-   Traps and snares account for `r ts_percent_raw`% of all MIS removal events and used at 52% of all properties.

-   Traps and snares had the largest amount of data loss. Data loss for snares and traps was largely due to inability to determine effort (trap nights) due to data entry errors. This was mostly due to implausible combinations of hours spent snaring, the number of snares set, and/or the number of snares checked, removed, or unset.

-   Aerial data appears to be of the highest quality with the lowest loss rate. Aerial data was lost due to data entry issues such as implausible number of aircraft flown, flight duration not recorded with a Hobbs meter, or the area of the property was not recorded or recorded as 1 acre.

-   Firearms had the largest loss of removal events largely due to duplicate records, hours spent using firearms recorded as 0 or longer than 24, or because their property ID (“AGRP_PRP_ID”) was not recorded. Luckily, the firearm events lost are contained to a select few properties.

-   After removing missing and errant data there are `r n_processed` properties left, which includes `r n_fb` Farm Bill properties. These properties represent `r process_left`% of all possible properties and `r fb_left`% of all Farm Bill properties.

|  |  |  |  |  |  |  |
|-----------|-----------|-----------|-----------|-----------|-----------|-----------|
|  | Raw MIS | Raw MIS | After Processing | After Processing | Percent Lost | Percent Lost |
...
| Total | `r raw_events` | `r raw_props` | `r processed_events` | `r processed_props` | `r percent_events_lost` | `r percent_props_lost` |

: One Table {#tbl-processedMIS tbl-colwidths="\[20, 12, 15, 12, 15, 12, 15\]"}

## Data meeting minimum criteria for density estimation

The final set of processing involves selecting properties that meet the minimum criteria for density estimation using a removal model. These criteria include:

1.  The time series is broken into 4-week chunks, or primary periods, and a primary period must have at least two removal events within it. I.e. any primary period with one event was discarded. Additionally, a property must have at least two good primary periods in its time series (i.e. primary periods with at least two removal events).

2.  We condition on first positive capture. I.e. within a property, we drop all primary periods at the beginning of a property’s time series that have 0 total pigs removed. This step was added to aid fitting the Bayesian model. Early attempts to fit the removal model without this step proved challenging, and through our simulation study and subsequent model fits we can get reliable density estimates.
